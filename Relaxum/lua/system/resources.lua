...
function M.unloadSpriteSheets( path )
    SHEETS[ path .. ".png" ] = nil
end
...
function M.loadPolysSheet( path )
    ...
    for _, object in ipairs( data["rigidBodies"] ) do
        ...
        local size = M.getSpriteConfig( object.name ).spriteSourceSize

        if not size.width or not size.height then
            warn( "Please load the appropiate spritesheet before loading the poly physics object." )
        end
        ...
        for _, fixture in ipairs( object["polygons"] ) do
            ...
            for _, point in ipairs( fixture ) do
                ...
                finalFixture[#finalFixture + 1] = ( point.y * size.height ) - ( size.height / 2 )
            end
...
function M.loadFont( path, name, options )
    ...
    options           = options         or {}
    options.small     = options.small   or DEFAULT_FONT_SMALL
    options.medium    = options.medium  or DEFAULT_FONT_MEDIUM
    options.large     = options.large   or DEFAULT_FONT_LARGE

    local font = MOAIFont.new()
    font:load(path .. '.ttf')
    font:preloadGlyphs(glyphs, options.small)
    font:preloadGlyphs(glyphs, options.medium)
    font:preloadGlyphs(glyphs, options.large)
...
function M.getFont( name, size )
    ...
    if name == nil then
        return FONTS[DEFAULT_FONT][size]
    elseif not FONTS[name] or not FONTS[name][size] then
